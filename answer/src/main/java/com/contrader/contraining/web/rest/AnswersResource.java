...
public class AnswersResource {
    ...
    @Timed
    public ResponseEntity<List<AnswersDTO>> getByQuestion(@PathVariable("id") Long idQuestion) {
        log.debug("CONTROLLER: retrieving answers for question with id " + idQuestion);

        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, "AnswersByUser"))
            .body(answersService.findByQuestion(idQuestion));

    }
    @GetMapping("/answersByUser/{id}")
    @Timed
    public ResponseEntity<List<AnswersDTO>> getByUser(@PathVariable("id") Long idUser) {
        log.debug("CONTROLLER: retrieving answers for user with id " + idUser);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, "AnswersByUser"))
            .body(answersService.findByUser(idUser));
    }
    @GetMapping("/answersByUserAndQuestion/{idUser}and{idQuestion}")
    @Timed
    public ResponseEntity<List<AnswersDTO>> getByQuestionAndUser(@PathVariable("idUser") Long idUser, @PathVariable("idQuestion") Long idQuestion) {
        log.debug("CONTROLLER: retrieving answers for user with id " + idUser);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, "AnswersByQuestionUser"))
            .body(answersService.findByQuestionAndUser(idQuestion, idUser));
    }

    @GetMapping("/sometext/{varText}")
    @Timed
    public ResponseEntity<String> getSomeText(@PathVariable("varText") String text) {
        log.debug("Getting some text from answer micro");
        return ResponseEntity.ok()
        .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, "answersOK"))
        .body(answersService.getText(text));
    }
    ...
    @PostMapping("/answers")
    @Timed
    public ResponseEntity<AnswersDTO> createAnswers(@Valid @RequestBody AnswersDTO answersDTO) throws URISyntaxException {
    ...
    @PutMapping("/answers")
    @Timed
    public ResponseEntity<AnswersDTO> updateAnswers(@Valid @RequestBody AnswersDTO answersDTO) throws URISyntaxException {
